...
/**
 * @title IPonderRouter
 * @notice Interface for the Ponder Router
...
interface IPonderRouter {
    ...
    /// @param amountToken Amount of tokens added
    ...
    /// @param path Array of addresses defining the swap route
    ...
    /// @param recipient Intended recipient of refund
    /// @param amount Failed refund amount
    /// @param reason Failure details
    event ETHRefundFailed(address indexed recipient, uint256 amount, bytes reason);
    ...
    event PriceImpactWarning(uint256 inputAmount, uint256 outputAmount, uint256 priceImpact);

    /// @notice Emitted when ETH for exact tokens swap begins
    /// @param sender Address initiating the swap
    /// @param ethValue Amount of ETH sent
    /// @param expectedOutput Expected token output amount
    /// @param deadline Transaction deadline
    event SwapETHForExactTokensStarted(
        address indexed sender, uint256 ethValue, uint256 expectedOutput, uint256 deadline
    );
    ...
    /// @dev Handles ETH wrapping and optimal amount calculations
    /// @param token Token to pair with ETH
    /// @param amountTokenDesired Desired amount of tokens to add
    /// @param amountTokenMin Minimum amount of tokens to add (slippage protection)
    ...
    /// @notice Removes liquidity from an ETH/token pair
    /// @dev Burns LP tokens and returns underlying assets, including unwrapping ETH
    /// @param token Token paired with ETH
    /// @param liquidity Amount of liquidity tokens to burn
    /// @param amountTokenMin Minimum amount of tokens to receive (slippage protection)
    ...
    /// @dev Handles tokens that charge fees on transfer
    /// @param token Token paired with ETH
    /// @param liquidity Amount of liquidity tokens to burn
    /// @param amountTokenMin Minimum amount of tokens to receive (slippage protection)
    /// @param amountETHMin Minimum amount of ETH to receive (slippage protection)
    ...
    /// @dev Handles ETH wrapping and refunding excess
    ...
    /// @notice Swap exact tokens for tokens supporting fee-on-transfer
    /// @dev Handles tokens that take fees on transfer
    /// @param amountIn Exact input token amount
    ...
    /// @notice Swap exact ETH for tokens supporting fee-on-transfer
    ...
    /// @dev Transaction exceeded time limit
    error Locked();
    ...
    /// @dev Output below minimum
    error InsufficientAAmount();
    /// @dev First token amount too low
    error InsufficientBAmount();
    /// @dev Second token amount too low
    error InsufficientAmount();
    /// @dev Generic amount too low
    error InsufficientInputAmount();
    /// @dev Input amount too low
    error ExcessiveInputAmount();
    /// @dev Input exceeds maximum
    error InvalidAmount();
    /// @dev Amount validation failed
    error ZeroOutput();
    /// @dev Output calculated as zero
    ...
    /// @dev Pool liquidity too low
    error ExcessivePriceImpact(
    /// @dev Price impact too high
    uint256 impact);
    /// @dev Impact in basis points
    ...
    error InvalidPath();
    /// @dev Swap path is invalid
    error IdenticalAddresses();
    /// @dev Attempting same-token swap
    error PairNonexistent();
    /// @dev Trading pair not found
    error PairCreationFailed();
    /// @dev Failed to create pair
    ...
    /// @dev ETH amount invalid
    error RefundFailed();
    /// @dev ETH refund failed
    error UnwrapFailed();
    /// @dev KKUB unwrap failed
    ...
    error ZeroAddress();
    /// @dev Invalid zero address
    error TransferFailed();
    /// @dev Token transfer failed
    error ApprovalFailed();
    /// @dev Token approval failed
    error InsufficientKkubBalance();
    /// @dev Not enough KKUB
    error KKUBApprovalFailure();
    /// @dev KKUB approval failed
}
