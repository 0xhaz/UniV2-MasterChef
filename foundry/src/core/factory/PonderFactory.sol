...
import { PonderFactoryStorage } from "src/core/factory/storage/PonderFactoryStorage.sol";
import { PonderFactoryLib } from "src/core/factory/libraries/PonderFactoryLib.sol";
...
contract PonderFactory is IPonderFactory, PonderFactoryStorage {
    ...
    /// @param ponder_ Address of the Ponder token contract
    constructor(address feeToSetter_, address launcher_, address ponder_) {
    ...
    /// @notice Returns protocol governance token address
    /// @return Address of the Ponder token contract
    function ponder() external view returns (address) {
        return _ponder;
    }

    /// @notice Returns pending launcher update if in progress
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        _getPair[token1][token0] = pair; // Populate reverse mapping
        _allPairs.push(pair);
