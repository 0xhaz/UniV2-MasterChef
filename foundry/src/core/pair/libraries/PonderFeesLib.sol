...
library PonderFeesLib {
    ...
    /// @notice Calculates the protocol and creator fees based on token type
    ...
    /// @return protocolFee Amount allocated to the protocol
    /// @return creatorFee Amount allocated to the token creator (if applicable)
        ...
        returns (uint256 protocolFee, uint256 creatorFee)
    {
        ...
        try ILaunchToken(tokenAddress).isLaunchToken() returns (bool isLaunch) {
            if (isLaunch && ILaunchToken(tokenAddress).launcher() == launcherAddress) {
                if (isPonderPair) {
    ...
    /// @return creatorFee Calculated creator fee
    /// @return creator Address of the token creator (zero if not applicable)
    function calculateAndReturnProtocolFee(
        ...
        internal
        view
        returns (uint256 protocolFee, uint256 creatorFee, address creator)
    {
    ...
    /// @return newAccumulatedFee Updated accumulated fee after adding protocol fee
    function handleTokenFee(
        ...
        internal
        returns (uint256 newAccumulatedFee)
    {
        ...
        returns (bool)
    {
        ...
        unchecked {
            ...
            // 1000 represents the total parts (1.000), subtracting 3 for the 0.003 fee
    ...
    /// @param accumulatedFee0 Accumulated protocol fee for token0
    /// @param accumulatedFee1 Accumulated protocol fee for token1
    /// @return isValid Whether sync operation is valid
