    ...
    ReentrancyGuard
{
    ...
    /// @dev Returns immutable factory address
    ...
    /// @dev Retrieved from factory
    /// @return PONDER token contract address
    ...
    function initialize(address token0_, address token1_) external {
        // Combine conditions for gas efficiency
    ...
    function mint(address to) external nonReentrant returns (uint256 liquidity) {
        ...
        if (currentSupply == 0) {
            ...
            if (
                amount0 < PonderPairTypes.MINIMUM_LIQUIDITY
            ...
            // For subsequent mitns, calculate liquidity proportional to existing supply
            liquidity = Math.min(
                (amount0 * currentSupply) / reserve0_, (amount1 * currentSupply) / reserve1_
            );
        }
    ...
    function burn(address to) external nonReentrant returns (uint256 amount0, uint256 amount1) {
        ...
        // Calculate token amounts to return based on liquidity burned
        ...
        // Transfer tokens to recipient (after state changes)
    ...
    /// @dev Supports flash swaps via a callback
        ...
        nonReentrant
    {
        ...
        // Flash loand protection - update reserves before external calls
        _update(
    ...
    function _handleFees(PonderPairTypes.SwapState memory state) private {
        // Cache storage value
        address token0_ = _token0;
        address token1_ = _token1;
