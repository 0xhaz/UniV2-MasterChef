...
interface ILaunchToken {
    ...
    event VestingInitialized(
        address indexed creator, uint256 amount, uint256 startTime, uint256 endTime
    );
    ...
    /// @dev Returns all relevant vesting parameters in a single call
    function getVestingInfo()
        external
        view
        returns (uint256 total, uint256 claimed, uint256 available, uint256 start, uint256 end);
    ...
    /// @param amount Total amount of tokens to be vested
    /// @dev Can only be called once by the launcher
    function setupVesting(address _creator, uint256 amount) external;
    ...
    /// @param _kubPair Address of the token-KUB pair
    /// @param _ponderPair Address of the token-PONDER pair
    /// @dev Can only be called by the launcher
